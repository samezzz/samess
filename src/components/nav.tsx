		...
		<nav className="py-10 mx-auto mt-8 md:mt-12 ">
			<div
				className={`flex h-5 items-center space-x-4 text-sm ${
					isScrolled
						? "glassmorphism px-2 py-3 transition-all duration-500 ease-in-out"
						: ""
				} rounded-full`}
			>
				{nav.map((navItem, index) => (
					<React.Fragment key={index}>
						<Link
							className={cn(
								"text-[#737373] hover:text-[#1C1C1C] hover:dark:text-primary transition-all duration-500 ease-in-out hover:font-semibold",
